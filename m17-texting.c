...
{
	const float scale = 7168.0f * gain * sqrtf(10.0f);
...
void generate_baseband(uint8_t phase_inv, float gain)
{
	...
	filter_symbols(NULL, NULL, rrc_taps_10, phase_inv, gain);
...
void button_press(GtkButton *button, gpointer user_data)
{
	...
	size_t max = sizeof(full_packet_data) - 4; // deduct 1 byte type, 1 null term ,and 2 bytes CRC
	size_t len = strnlen(settings.msg, max);
	memcpy(&full_packet_data[1], settings.msg, len);
	num_bytes = len + 2; // 0x05 and 0x00
	...
	uint16_t type = 0;
	encode_callsign_value(&dst_enc, settings.dst_raw);
	encode_callsign_value(&src_enc, settings.src_raw);
	...
	for (uint16_t i = 0; i < strlen(settings.msg); i++)
	...
	usleep(500000); // wait a bit for the transmission to complete
	setRTS(ser, 0); // release PTT
